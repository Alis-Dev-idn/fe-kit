...
│   ├── components/
│   │   ├── Table.tsx                # Ready-to-use table component
│   │   ├── Table.Pagination.tsx     # Pagination sub-component
│   │   ├── Table.ColumnToggle.tsx   # Column visibility toggle
│   │   ├── Table.ExportButton.tsx   # Export button
│   │   ├── TableSkeleton.tsx        # Default loading skeleton
│   │   ├── TableEmpty.tsx           # Default empty state
...
Ready-to-use component that wraps `useTable` internally. Handles all rendering including sticky header, frozen columns, mobile card view, expandable rows, pagination, column toggle, and export.

```typescript
interface TableProps<T extends object> extends UseTableOptions<T> {
  // Built-in UI features
  showPagination?: boolean              // default: true
  showColumnToggle?: boolean            // default: false
  showExport?: boolean                  // default: false
  exportFormats?: ("csv" | "excel")[]   // default: ["csv", "excel"]

  // Styling — all Tailwind compatible
  className?: string                    // table wrapper
  headerClassName?: string              // thead row
  rowClassName?: string | ((row: T) => string)  // tr, supports dynamic per row
  cellClassName?: string                // td
  mobileCardClassName?: string          // mobile card wrapper
  paginationClassName?: string          // pagination wrapper
  columnToggleClassName?: string        // column toggle wrapper
  exportClassName?: string              // export button wrapper
}

// Compound component
const Table: <T extends object>(props: TableProps<T>) => React.ReactElement & {
  Pagination: typeof TablePagination
  ColumnToggle: typeof TableColumnToggle
  ExportButton: typeof TableExportButton
}
```
...
```
[← Prev]  Page 1 of 5 (42 total)  [Next →]   Show: [10 ▾]
```
...
Default UI: dropdown button showing all columns with checkboxes.
...
Default UI: dropdown button with CSV/Excel options.

---

### Usage — Simple (built-in everything)

```typescript
import { Table } from "@alisdev/fe-kit"
...
export function UsersTable() {
  return (
    <Table<IUser>
      fetchFn={(params) => UserService.findAll(params)}
      columns={[
        { key: "firstName", label: "First Name", sortable: true, mobilePriority: 1, pinned: "left", width: "150px" },
        { key: "email", label: "Email", filterable: true, mobilePriority: 2 },
        { key: "age", label: "Age", sortable: true, mobilePriority: 3 },
        { key: "createdAt", label: "Created At", sortable: true },
        {
          key: "actions",
          label: "Actions",
      ...
      expandable={{
        renderAsync: async (row) => {
          const orders = await OrderService.findByUser(row._id)
          return (
            <Table<IOrder>
              data={orders}
              columns={[
      ...
      mobileCardClassName="shadow-sm rounded-lg border border-slate-200 mb-3"
    />
  )
}
```

### Usage — Custom layout (sub-components)
  ...
  return (
    <div>
      ...
      <div className="flex justify-between mb-4">
        ...
        />
        <div className="flex gap-2">
          <Table.ColumnToggle table={table} label="Show/Hide Columns" />
          <Table.ExportButton table={table} formats={["csv"]} />
        </div>
      </div>

      {/* Bulk actions */}
      {table.selectedRows.length > 0 && (
        <div className="mb-2 p-2 bg-blue-50 rounded flex items-center gap-3">
          <span>{table.selectedRows.length} selected</span>
          <button onClick={() => bulkDelete(table.selectedKeys)}>Delete</button>
        </div>
      )}
...
2. Export `useTable`, `Table`, `exportCsv`, `exportExcel` from `src/index.ts`
3. `Table` component uses `useTable` internally — no duplicate logic
